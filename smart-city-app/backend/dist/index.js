...
const mobility_1 = require("./mobility");
...
class GeospatialService {
    ...
    async geocode(address) {
        try {
            const response = await axios_1.default.get(`${this.nominatimBaseUrl}/search`, {
                params: {
                    q: address,
                    format: 'json',
                    limit: 1,
    ...
    async searchPlaces(query, lat, lng, radius = 1000) {
        try {
            const amenityMap = {
                'pharmacy': 'pharmacy',
                'pharmacies': 'pharmacy',
        ...
        out center;
      `;
            const response = await axios_1.default.post(this.overpassBaseUrl, overpassQuery, {
                headers: {
    ...
    async reverseGeocode(lat, lng) {
        try {
            const response = await axios_1.default.get(`${this.nominatimBaseUrl}/reverse`, {
                params: {
                    lat,
                    lon: lng,
                    format: 'json',
    ...
    async getPharmacies(lat, lng, radius = 1000) {
        try {
            const overpassQuery = `
        [out:json][timeout:25];
        (
    ...
    async getRestaurants(lat, lng, radius = 1000) {
        try {
            const overpassQuery = `
        [out:json][timeout:25];
        (
...
class AIService {
    async processQuery(message, userLocation) {
    ...
    async getDirections(from, to, userLocation) {
        try {
            ...
            const distance = geospatialService.calculateDistance(userLocation.lat, userLocation.lng, toResult.lat, toResult.lng);
            return {
    ...
    async searchServices(serviceType, userLocation) {
        try {
            ...
            const markers = places.slice(0, 5).map((place) => ({
            ...
            const distance = geospatialService.calculateDistance(userLocation.lat, userLocation.lng, nearest.lat || nearest.center?.lat, nearest.lon || nearest.center?.lon);
            return {
    ...
    async getLocationContext(userLocation) {
        try {
            ...
            const context = [];
            if (address)
                context.push(`You're currently at ${address}`);
            if (pharmacies.length > 0)
                context.push(`${pharmacies.length} pharmacies nearby`);
            if (restaurants.length > 0)
                context.push(`${restaurants.length} restaurants nearby`);
            return {
...
app.get('/api/places/search', async (req, res) => {
    ...
    try {
        ...
        const places = await geospatialService.searchPlaces(query, parseFloat(lat), parseFloat(lng), parseInt(radius));
        const formattedPlaces = places.map((place) => ({
            ...
            position: [place.lat || place.center?.lat, place.lon || place.center?.lon],
            description: place.tags?.opening_hours || place.tags?.cuisine || 'No additional info',
            distance: geospatialService.calculateDistance(parseFloat(lat), parseFloat(lng), place.lat || place.center?.lat, place.lon || place.center?.lon).toFixed(1) + ' km'
        }));
        res.json({
...
app.get('/api/geocode', async (req, res) => {
    ...
    try {
        ...
        const result = await geospatialService.geocode(address);
        if (result) {
            res.json(result);
        }
        else {
...
app.get('/api/routes', async (req, res) => {
    ...
    try {
        if (mode === 'public_transport') {
            ...
            const transportRoute = await transport_1.publicTransportService.planJourney([fromCoords[0], fromCoords[1]], [toCoords[0], toCoords[1]]);
            if (transportRoute) {
                res.json({
                    id: Date.now().toString(),
            ...
            const fromCoords = from ? from.toString().split(',').map(Number) : [47.4979, 19.0402];
            const toCoords = to ? to.toString().split(',').map(Number) : [47.5079, 19.0502];
            const bikeRoute = await mobility_1.sharedMobilityService.getBikeRoute([fromCoords[0], fromCoords[1]], [toCoords[0], toCoords[1]]);
            if (bikeRoute) {
                res.json({
                    id: Date.now().toString(),
                    mode: 'cycling',
...
app.get('/api/transport/stops', async (req, res) => {
    ...
    try {
        ...
        const stops = await transport_1.publicTransportService.getNearbyStops(parseFloat(lat), parseFloat(lng), parseInt(radius));
        res.json({
            stops,
            count: stops.length,
            location: { lat, lng },
...
app.get('/api/transport/arrivals/:stopId', async (req, res) => {
    ...
    try {
        const arrivals = await transport_1.publicTransportService.getStopArrivals(stopId);
        res.json({
            stopId,
            arrivals,
...
app.get('/api/transport/disruptions', async (req, res) => {
    try {
        const disruptions = await transport_1.publicTransportService.getDisruptions();
        res.json({
            disruptions,
...
app.get('/api/mobility/bikes', async (req, res) => {
    ...
    try {
        ...
        const bikeStations = await mobility_1.sharedMobilityService.getNearbyBikeStations(parseFloat(lat), parseFloat(lng), parseInt(radius));
        res.json({
...
app.get('/api/mobility/summary', async (req, res) => {
    try {
        const summary = await mobility_1.sharedMobilityService.getBikeAvailabilitySummary();
        res.json({
            summary,
            timestamp: new Date().toISOString()
        });
...
app.get('/api/weather/current', async (req, res) => {
    try {
        const weather = await weather_1.weatherService.getCurrentWeather();
        if (weather) {
            const context = weather_1.weatherService.getWeatherContext(weather);
            res.json({
                weather,
                context,
                timestamp: new Date().toISOString()
...
app.get('/api/weather/forecast', async (req, res) => {
    try {
        const forecast = await weather_1.weatherService.getWeatherForecast();
        res.json({
            forecast,
...
app.get('/api/weather/alerts', async (req, res) => {
    try {
        const alerts = await weather_1.weatherService.getWeatherAlerts();
        res.json({
            alerts,
            count: alerts.length,
            timestamp: new Date().toISOString()
